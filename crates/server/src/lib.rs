//! `pr0_runtime`: the pr0former application runtime as a library.
//!
//! This crate owns everything the standalone server does — database setup, the
//! engine/orchestration worker, and the Axum HTTP/WebSocket router.
//!
//! Hosts embed it through one lifecycle API:
//!
//! ```no_run
//! # async fn example() -> Result<(), String> {
//! let config = pr0_runtime::RuntimeConfig::embedded("/tmp/pr0former-data");
//! let runtime = pr0_runtime::start(config).await?;
//! let ready = runtime.readiness(); // loopback URL + private owner session
//! println!("serving on {}", ready.url); // the session stays out of logs
//! runtime.host_on_lan(8443).await?; // opt-in local-network hosting
//! runtime.shutdown().await?; // ordered, idempotent, never exits the process
//! # Ok(()) }
//! ```
//!
//! [`start`] reads no command-line arguments and no `PR0_*` variables, keeps no
//! global state, and never calls `std::process::exit`, so several runtimes can
//! run side by side in one process. [`run`] is the `pr0-server` process host
//! that translates CLI/environment settings into a [`RuntimeConfig`], and
//! `--desktop` is now only a stdin/stdout adapter over the same API.
//!
//! See `docs/IPAD_RUNTIME_PLAN.md` for the broader host-adapter plan. Routes and
//! DSP/scheduling semantics are unchanged by this split.
mod accounts;
mod audio;
mod bind;
pub mod build_info;
...
mod media;
mod monitor_packets;
/// The platform native-audio backend boundary: enumerated devices on desktop
/// and the standalone server, logical routes on iOS/iPadOS.
mod native_audio;
mod node_io;
mod osc;
mod output_buffer;
...
pub use config::{
    AudioPolicy, AudioRoutes, CaptureAuthorization, CaptureSupport, ListenerPolicy,
    RouteDescription, RuntimeConfig, RuntimeMode,
};
pub use import::{
    AudioImporter, DecodedAudio, ImportError, ImportLimits, ImportSource, ImporterChain,
};
pub use runtime::{AUDIO_TRANSITION_TIMEOUT, Readiness, RuntimeHandle, start};
pub use settings::{AudioPreferences, preferences as audio_preferences};
...
async fn get_project(
    ...
    let rate = settings::read(&app.config).sample_rate;
    let config = app.config.clone();
    tokio::task::spawn_blocking(move || samples::cache_project(&config, &prepared, rate))
        .await
        .map_err(internal)?
        .map_err(bad)?;
    app.logs
...
async fn clear_loop(
    ...
    if app.graph.lock().unwrap().as_deref() == Some(&id) {
        ...
        let root = app.config.loops_dir();
        tokio::task::spawn_blocking(move || {
            loops::clear_saved(&root, &id, &command.node, command.track)
        })
        .await
        .map_err(internal)?
        .map_err(bad)?;
...
async fn devices(State(app): State<App>, headers: HeaderMap) -> Api<Json<Value>> {
    ...
    let mut inventory = {
        ...
        let settings = if admin {
            settings::discover(&app.config).await?
        } else {
            settings::read(&app.config)
        };
        let config = app.config.clone();
        tokio::task::spawn_blocking(move || audio::device_inventory(&config, settings.sample_rate))
            .await
            .map_err(internal)?
    };
    let (tx, rx) = oneshot::channel();
...
async fn enable_engine(app: &App, id: &str, enabled: bool) -> Api<()> {
    ...
    send(
        app,
        audio::Command::Enable(id.into(), enabled, settings::read(&app.config), tx),
    )?;
...
async fn start_graph(app: &App, id: &str) -> Api<()> {
    ...
    let prepared = project.clone();
    let config = app.config.clone();
    let engine = tokio::task::spawn_blocking(move || samples::prepare(&config, &prepared))
        .await
        .map_err(internal)?
        .map_err(bad)?;
...
async fn latency_test(
    ...
    if c.enabled {
        if !settings::read(&app.config).interfaces.iter().any(|i| i.enabled) {
...
/// Baseline response hardening applied to every route this runtime serves.
///
/// `X-Frame-Options` and `X-Content-Type-Options` are absolute; `Referrer-Policy`
/// is a floor. Each header is only inserted when the handler did not already
/// set one, so a response with a stricter requirement — the one-time session
/// handoff, which sets `no-referrer` — keeps its own value.
///
/// Applied with [`Router::layer`] *after* every route and the fallback are
/// registered: axum wraps the routes that exist when the layer is added, so
/// applying it to an empty router silently protects nothing.
...
/// The application, assembled but not yet listening: shared state, the fully
/// routed axum application, and the private owner session when this runtime is
/// embedded. Hosts choose their own transport on top of this.
...
/// Opens and migrates the database, starts the engine and its workers, creates
/// the private owner session for embedded runtimes, and builds the
/// HTTP/WebSocket router.
///
/// Everything here comes from the typed [`RuntimeConfig`]: no environment
/// variable, command-line argument or process-global is consulted, and no
/// global singleton is written, so independent runtimes can be assembled side
/// by side in one process. Must be called from within a Tokio runtime — the
/// OSC, conductor-MIDI and autosave workers spawn tasks.
pub(crate) fn assemble(config: Arc<RuntimeConfig>) -> Result<Assembled, String> {
    std::fs::create_dir_all(&config.data_dir)
        .map_err(|e| format!("Create data directory: {e}"))?;
    let mut db =
        Connection::open(config.database_path()).map_err(|e| format!("Open database: {e}"))?;
    ...
    subgraphs::migrate(&db).map_err(|e| format!("Subgraph library migration: {e}"))?;
    revisions::migrate(&db).map_err(|e| format!("Revision save migration: {e}"))?;
    let session = config
        .is_embedded()
        .then(|| runtime::private_owner_session(&mut db))
        .transpose()?;
    let (events, _) = broadcast::channel(128);
    let media = Arc::new(media::Media::new());
    let logs = Arc::new(settings::Logs::default());
    let osc = osc::Runtime::load(&config);
    ...
    let assets = config.web_root().to_path_buf();
    let router = Router::new()
        ...
        .layer(axum::extract::DefaultBodyLimit::max(4 * 1024 * 1024))
        .fallback_service(
            ServeDir::new(&assets).not_found_service(ServeFile::new(config.web_index())),
        )
        // Applied last on purpose. `Router::layer` wraps the routes and the
        // fallback that already exist, so a layer added before the routes
        // reaches none of them. Every API route, the WebSocket upgrade and the
        // static interface are all registered above this line.
...
/// The `pr0-server` process host: the only place that reads command-line
/// arguments and `PR0_*` variables.
///
/// It turns those into a [`RuntimeConfig`], starts the shared runtime, and
/// awaits it. `--desktop` selects the legacy child-process protocol, which is
/// now only a stdin/stdout adapter over the same embeddable API.
pub async fn run() -> std::process::ExitCode {
    ...
    tracing_subscriber::fmt()
        ...
        .init();
    let embedded = std::env::args().any(|arg| arg == "--desktop");
    if embedded {
        // `from_env` leaves the listener policy empty for embedded runtimes, so
        // PR0_BIND/PR0_HOST/PR0_PORT and the TLS pair are deliberately ignored
        // here: no privileged ports and no LAN bypass of the private listener.
        return desktop::run(RuntimeConfig::from_env(RuntimeMode::Embedded)).await;
    }
    let mut config = RuntimeConfig::from_env(RuntimeMode::Server);
    // Managed-certificate discovery and the legacy `.local/ssl` migration are
    // process-host policy, so they stay above the runtime boundary.
    let tls_config = tls::Config::load();
    config.listener.tls = tls_config.pair;
    config.listener.port = tls_config.port;
    let handle = runtime::start(config).await.expect("Start pr0former");
    println!("pr0former listening on {}", handle.readiness().url);
    if let Some(redirect) = &handle.readiness().redirect_url {
        println!("HTTP redirect listening on {redirect}");
    }
    let served = handle.serving().await;
    let stopped = handle.shutdown().await;
    served.expect("Serve pr0former");
    stopped.expect("Stop pr0former");
