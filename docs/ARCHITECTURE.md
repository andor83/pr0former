...
asset ID, sample name and optional nickname). The catalog exposes a numeric Index
parameter/input and Sample ID output. Indices are zero-based, fractional values
round down, and empty/out-of-range selections output 0. Manual orange buttons use
the normal validated parameter endpoint; connected Index controls remain read-only.
Configuration belongs in the modal, and telemetry never becomes list/undo data.

`sample`, `poly_sampler` and `granular_synth` expose a numeric `sample_id` input. `granular_field` has no `sample_id` port: it reuses `sample_choices` as its ordered slot list (at most 8, validated in `validate_flat_inner`), exposes a connectable `sample_N` asset-id parameter per configured slot (0 uses the list entry; edges to unconfigured slots are rejected), and preparation always loads every list entry into the bank, plus the project shortlist when any `sample_N` is cable-driven. Subgraph export/import and `.pr0` bundles remap nonzero `sample_N` values alongside `sample_choices`.
Without a cable they use the existing structural `asset` setting. With a cable,
preparation loads the default plus the graph's shortlist assets that match the
consumer's channel count, retaining the existing 256 MB total prepared-audio limit.
No channel conversion is implicit. Runtime changes swap preallocated sample
buffers and reset the playback position/voices; selection does not trigger a note.
Unknown IDs select silence and expose `_sample_missing`; `_sample_id` reports the
loaded asset. Root MIDI note stays at the sampler's configured/connected value.
Sample player still follows show transport; the two instruments accept graph MIDI
while the development engine runs. Convolution and phase-vocoder assets retain
their existing preparation-only assignment behavior.

The server validates list ownership, size, IDs, text bounds and project sample
availability even when the engine is disabled. Subgraph library saves bundle
shortlist audio and remap its IDs when inserted into another project. Bank/list
identity participates in graph-state compatibility so replacement does not retain
...
Drum pads expose six numeric `trigger_1`–`trigger_6` inputs in pad order.
A zero-to-nonzero transition (either sign) sends the configured pad note on
the configured MIDI channel at velocity 100. Returning to zero sends note-off
and rearms that input. Held nonzero values and sign changes do not retrigger.
These messages share the existing typed MIDI and per-pad velocity pulse outputs.

Knobs and Sliders contain 1–8 MIDI CC controls. Each control has a channel and CC number, a numeric outlet, and (for Sliders) a matching numeric input. Received MIDI is forwarded unchanged; GUI gestures emit assigned CCs. The browser coalesces rapid gesture updates to the newest unsent value per node control, so a slow control request cannot drain obsolete positions after a sweep. Slider values apply range, step and decimal rounding in DSP. Connected slider inputs are read-only in the UI and controller API. Live values and learn state use fixed prepared arrays; telemetry serialization occurs outside rendering. MIDI Learn captures the next incoming CC and the initiating browser saves its channel/number as configuration. GUI values are transient; learned assignments and display options are project data.

Local audio input starts the shared WebRTC/Opus microphone session automatically while the engine is enabled, selecting the user's first explicitly assigned input. Microphone permission is still required. One input uplink is available per project/user session; a node accepts only one sender. The on-node microphone button controls the `mute` parameter (default 0); a positive connected value silences every channel in DSP and locks manual editing. Muting retains capture and the peer connection. Device and machine-name choices live locally in the modal. Tauri supplies its hostname; browsers use an editable local device name because browser APIs do not expose hostnames. `GET projects/:id/media` exposes transient, project-authorized source identity and connection state to all members; the graph and Monitor display user/machine names and sampled per-channel output levels. Live source metadata is removed when the sender lease ends and never creates a project revision. Node-to-user bindings are stored separately in `Project.local_audio_assignments`, persist in revisions and do not depend on score-part assignments. Only owners/editors may change them, and targets must be ensemble members. Unassigned nodes do not capture. Even owners/editors can send audio or mute only their own assigned inputs; device choices remain local to that browser/app instance. Reassignment cancels the old sender, clears its queued input at graph replacement, and checks the project/user binding on queued packets. A second instance can explicitly choose “Use this device” to move its own capture session. The microphone is gray unless a connected sender has delivered decodable audio within two seconds (source snapshots refresh every 1.5 seconds).
...
`--uas` pulls the current branch from its configured upstream with `git pull --ff-only`, disabling automatic rebase/stash. It invokes the newly pulled script's `--update` followed by `--start`, preserving host/port overrides and the existing saved-script/environment behavior. Any pull/build failure prevents launch; it does not stop existing processes or configure startup services. Its server process uses the usual manual PID tracking. `--uar` is the same pull and `--update` followed by `--restart`. On macOS, `sign_server_binary` treats codesign's `errSecInternalComponent` as a locked or unauthorized login Keychain: in a terminal it offers `security unlock-keychain` and then `scripts/setup-macos-signing.sh` before retrying; without a terminal it warns and keeps the ad-hoc signature.
...
Manual `--start` launches record their PID, owner, and process start time under `.local/manual-runs`. `--stop` checks that identity before sending SIGTERM and waits up to ten seconds per process. Stale records are discarded. Only launches made with tracking enabled are covered. `--stop` additionally stops the installed startup service (`launchctl bootout` on macOS, `systemctl --user stop` on Linux) without disabling it. `--restart` stops manual runs, then restarts the service (`launchctl kickstart -k`, or `bootstrap` when it was booted out; `systemctl --user restart` on Linux); with no service installed it re-executes `--start` in the foreground. The startup generator normalizes a bare port answer to `0.0.0.0:PORT`, since the server rejects `PR0_BIND` without a host.
...
`synth` and `fm_synth` accept numeric MIDI `pitch`, `velocity`, `gate`, `trigger` and pitch-specific `note_off` inputs. They also accept direct score events through the existing part/note-instance scheduler. MIDI controls use the shared edge detector and a reserved voice-owner scope; releasing a graph note cannot release a score-owned note at the same pitch. Repeated MIDI pitches release oldest-held-first. Gate is a monophonic fallback when explicit trigger inputs are absent; unconnected velocity defaults to 100. Both use 64 fixed voice slots, reuse silent slots first and steal the quietest at capacity. Compatible live replacements preserve graph-held voices and both oscillator phases while scheduled voices still migrate only for surviving parts. Every voice runs the shared linear ADSR (`attack`, `decay`, `sustain`, `release`, the same parameters and ranges as the ADSR node and `poly_sampler`); a sustain of 0 gives the older one-shot contour. Projects saved before the full envelope only had `decay` and `release`, where a positive decay fell to silence: `Project::upgrade_legacy_envelopes`, applied when the server loads a project, adds `sustain: 0` to such nodes so they sound unchanged. Instruments (`synth`, `fm_synth`, `poly_sampler`, `granular_synth`, `granular_cloud`) publish `_envelope` (loudest live voice), `_voices`, `_held`, and a five-entry newest-first ring of dispatched note events (`_note_count`, `_recent_N_status/_data1/_data2`); the client's `InstrumentFace` draws the sample name, the envelope with that live trace, and the recent notes on the node, and the node modal offers the same drag-handle envelope editor as the ADSR node.
...
`granular_synth` uses project sample preparation, caches, integer root defaults and subgraph sample bundling. It accepts the standard five graph MIDI controls, including repeated-pitch note-off handling, from Piano/MIDI/Part MIDI. Sixteen fixed voice records drive a fixed 128-grain pool; capacity steals bounded slots. Births latch sample position plus deterministic random spray, pitch ratio and duration. Hann grains read the wrapped sample with linear interpolation and share channel timing; density compensates average overlap gain. Released voices stop spawning grains; every voice's gain follows its own linear ADSR (`attack`, `decay`, `sustain`, `release`), so existing grains fade over the release and the slot frees when the envelope ends. All storage is prepared and rendering performs no allocations, locks or I/O. Direct score-instrument scheduling is not added; scores use Part MIDI routing.

`granular_field` (`crates/dsp/src/granular_field.rs`) is a continuous multi-source cloud with no note, gate or MIDI inputs: its only ports are two audio inputs, `live_1` and `live_2`, plus `out`. Up to eight sample slots and the two live inputs sit on a field with `x`/`y` in −1..1; the `x`/`y` parameters place a control point and each grain picks its source at birth with probability `exp(-(distance/focus)^2)` from a single 128-grain pool whose grains record their source slot. `pitch` transposes grains in semitones or, with `randomize_pitch` above 0, sets a uniform per-grain range; every source has its own `tune` and `gain`. A live input joins the field only while its port has a binding; each bound input reserves a ten-second ring at prepare (`try_reserve_exact`, capped at 256 MiB across all fields) whose usable window is the live `live_N_buffer_ms`, and grain starts are clamped so no read crosses the write head at any increment. Every setting is an ordinary connectable parameter (none structural) read through indices resolved once at prepare; rendering never allocates (`render_allocations.rs`). Telemetry publishes `_x`, `_y`, `_source_1.._10_weight`, `_sample_N_missing`, `_grains` and `_live_N_fill`; compatible replacements swap the field state so captured audio survives a slot edit.
...
Score entry audition is an authorized request naming an already saved part/note, with an optional server-validated MIDI pitch (0–127) for an unsaved drag preview. Routing, velocity, mute and solo still come from the saved note and part. The worker sends scoped instrument notes plus staff-filtered Part MIDI events, then releases them after a bounded count of rendered samples. Replacement retires the current preview; unload clears it. This path does not use browser note-off timers or directly address legacy part MIDI/OSC device routes. Persisted part mute and exclusive-solo choices remove the affected prepared note and automation streams; replacement releases existing held notes. Special barline styles are validated layout metadata and do not change traversal.


Existing-note pointer gestures preview selected notes from a fixed score/mapping snapshot; only empty-space drags draw a selection rectangle. Pitch changes audition through the existing part/staff routes, with serialized browser requests coalescing pending movement. Release commits one edit; Escape, pointer cancellation, capture loss, window blur and playback start discard the position preview. The score editor disables entry/edit controls during playback and count-in, restoring them on pause/stop. This is an editor interaction lock; the preparation API still supports live project updates.
...
Node descriptors include optional authored `NodeDocumentation` (worked-example title, explanation, setup steps and a normal graph). Built-in entries live with `pr0-core` in `node_documentation.json`; every catalog kind has an example checked by the server graph validator. The guide uses Vue Flow, the normal node/edge components, dynamic node descriptors and the shared layout function, with a separate flow store and inactive telemetry. Example inspection cannot activate audio or mutate projects. Setup/architecture/script instructions are available in the standalone documentation view as well as in the app. See [DOCUMENTATION.md](DOCUMENTATION.md) for the authoring contract.

### Independent part player nodes

`part_player` selects a project-local `part_id`. Server preparation reuses the score sequencer’s notation interpretation, clips traversal to the part’s duration, then converts performer-pulse coordinates back to quarter-note beats. Prepared clips include staff channels, ties, articulation, dynamics, written-position MIDI automation, meter changes and traversal spans. Explicit graph playback ignores show mute/solo and launch queues. Score tempo marks do not control a clip: it follows the current engine BPM.

Three numeric inputs detect positive rising edges: `play` schedules one pass, `repeat` schedules looping, and `stop` cancels immediately (priority stop, repeat, play). Launch/relaunch waits for the next denominator beat of the graph metronome, without count-in. Already playing clips continue until their scheduled restart. Once started, local position integrates engine samples at current BPM, independent of show play/pause/seek/stop. While the show runs, launch detection shares the audible metronome’s written position and meter origin, including repeat jumps and meter changes. Otherwise it uses the free-running graph beat grid. A pending launch is fulfilled before accepting a new trigger on that same beat, so a periodic clock cannot starve playback. MIDI and numeric note outlets share the normal typed MIDI producer contract. Bar/beat reports written score coordinates through notation repeats; telemetry is never persisted.

All clip/event/automation storage is prepared outside rendering, with a graph-wide cap of one million prepared note/automation events. Sample processing only walks prepared events and bounded MIDI buffers. Stop, completion, looping and retrigger release held channel/pitch instances and reset automation pedals. Compatible graph replacements preserve position; changed/deleted clips release into surviving typed consumers. Project unload and MIDI route retirement use existing external-output cleanup. Physical MIDI timing remains a manual device path, not a sample-accurate hardware claim.

### Sampler voice envelopes and granular clouds

Polyphonic sampler voices each own the same fixed-size `envelope::Adsr` used by the ADSR node. `envelope_parameters` supplies shared metadata/ranges, and the Vue `EnvelopeGraph`/geometry/value reader supplies the compact display and editable modal graph. Each note starts a fresh envelope; note-off releases the oldest held matching pitch from its current level. Voice occupancy is separate from envelope amplitude, preserving zero-sustain and zero-level attack voices. Attack/decay/release times latch at stage entry, sustain changes use the existing smoothing, and global amplitude still scales the complete mix. Defaults (0/0/1/80 ms) retain legacy patches’ immediate attack and full sustain. One-shot samples may finish before their envelope. `_envelope` is the maximum active envelope, with `_voices` and `_held` read-only counts.
...
The Granular Field node modal puts a `GranularFieldPad` first (SVG, axes crossing at the centre, one draggable `role="slider"` circle per source sized by its live weight, a focus ring around the draggable control point, per-axis locks for cabled parameters, arrow-key nudges) with an Edit sources button opening `GranularFieldSources`, a second `<dialog>` stacked in the top layer that edits `sample_choices` and per-slot tune/gain and shows each live input's connection and buffer length. `web/src/granularField.ts` holds the geometry, the engine's weighting formula for previews, slot-parameter reordering and the descriptor filter used by `nodeDescriptor()` so only configured `sample_N` setters appear as ports; `GranularFieldFace` draws the compact field and source shares on the node.
