...
impl Iterator for Devices {
    ...
    fn next(&mut self) -> Option<Device> {
        while self.builtin_pos < BUILTINS.len() {
            ...
            // Modern Linux normally has no OSS compatibility device. Do not
            // open its ALSA plugin on every scan (it logs directly to stderr).
            if name == "oss" && !std::path::Path::new("/dev/dsp").exists() {
                continue;
            }
