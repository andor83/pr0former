...
impl Device {
    /// Construct an explicit ALSA PCM without scanning/opening unrelated PCMs.
    /// pr0former extension; capability validation happens when the PCM is used.
    pub fn from_pcm_name(name: &str) -> Self {
        Self {
            name: name.to_owned(),
            pcm_id: name.to_owned(),
