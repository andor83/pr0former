...
impl Device {
    /// pr0former extension: stable, opaque Windows audio endpoint identity.
    pub fn endpoint_id(&self) -> Result<String, DeviceNameError> {
        unsafe {
            let id = self.device.GetId().map_err(|e| DeviceNameError::from(BackendSpecificError { description:e.to_string() }))?;
            let result = id.to_string().map_err(|e| DeviceNameError::from(BackendSpecificError { description:e.to_string() }));
            Com::CoTaskMemFree(Some(id.0 as *mut _));
            result
        }
    }
