...
test('enabled graph runs audio and clocks while the editable show timeline stays stopped',async({page})=>{
  ...
  await page.goto('/')
  const monitorToggle=page.getByRole('button',{name:'Browser monitor',exact:true})
  await expect(monitorToggle).toBeDisabled()
  await expect(monitorToggle).toHaveAttribute('aria-pressed','false')
  const showBox=(await page.getByRole('button',{name:'Enable audio engine',exact:true}).boundingBox())!,toggleBox=(await monitorToggle.boundingBox())!
  expect(toggleBox.x).toBeGreaterThanOrEqual(showBox.x+showBox.width)
  ...
  const phoneShow=(await page.getByRole('button',{name:'Disable audio engine',exact:true}).boundingBox())!,phoneToggle=(await monitorToggle.boundingBox())!
  expect(phoneToggle.x).toBeGreaterThanOrEqual(phoneShow.x+phoneShow.width)
  expect(phoneToggle.x+phoneToggle.width).toBeLessThanOrEqual(390)
  await page.screenshot({path:'test-results/browser-monitor-toggle-phone.png'})
