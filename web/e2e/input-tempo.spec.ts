...
test('input device availability, local browser selection, and connected global tempo', async ({ page }) => {
  ...
  await expect(page.getByText('No local audio inputs are available or exposed.',{exact:false})).toBeVisible()
  await page.getByRole('button',{name:'Grant microphone access'}).click()
  await expect(page.getByRole('alert').filter({ hasText: 'Microphone permission is denied' }).first()).toBeVisible()
  ...
  await page.getByRole('button',{name:'Refresh local audio inputs'}).click()
  await expect(page.getByLabel('Local audio input device').locator('option')).toHaveCount(3)
  await page.getByLabel('Local audio input device').selectOption('mic-b')
  await page.getByRole('button',{name:'Close parameters'}).click()
  ...
  await page.getByLabel('Local audio input node',{exact:true}).selectOption('Browser')
  await expect(page.getByLabel('Local audio input device')).toHaveValue('mic-b')
  ...
  await page.getByRole('button',{name:'Refresh local audio inputs'}).click()
  await expect(page.getByLabel('Local audio input device').locator('option:checked')).toHaveText('Selected input unavailable — refresh and choose another')
