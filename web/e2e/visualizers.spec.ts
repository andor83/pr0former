...
test('visualizers preserve typed inputs and display multichannel block history, bins, and phase',async({page})=>{
  ...
  const sample=latest.sample,sequence=latest.visualizations['Audio display'].sequence,telemetrySequence=latest.sequence
  ...
  await page.screenshot({path:'test-results/visualizer-nodes.png',fullPage:true})
  ...
  await page.screenshot({path:'test-results/spectral-visualizer.png',fullPage:true})
  ...
  // Stop the show; the development graph keeps running so engine time still advances.
