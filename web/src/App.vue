...
async function saveScoreProject(next:Project) {
  if(!project.value||saving.value||active.value||!editable.value)throw new Error('Score editing is currently unavailable.')
  saving.value=true
  try {
...
function downloadXML() { if (!part.value || !project.value) return; const warnings=musicXMLExportWarnings(project.value); notice.value=warnings.join(' '); const url = URL.createObjectURL(new Blob([exportScoreMusicXML(project.value)], { type: 'application/xml' })); const link = document.createElement('a'); link.href = url; link.download = `${project.value.name}.musicxml`; link.click(); URL.revokeObjectURL(url) }
...
<template>
  ...
  <div v-else class="app-shell" :class="{ 'fullscreen-fallback': fullscreen, 'performance-mode': stage }">
    ...
    <template v-else>
      ...
      <div v-else-if="project && tab === 'score'" class="content-pane score-content"><div class="score-actions"><button class="button small" @click="downloadXML"><Download :size="14" /> MusicXML</button><label class="button small">Import MusicXML<input type="file" accept=".xml,.musicxml" style="display:none" :disabled="active || !editable" @change="uploadXML"></label><button v-if="project.mode !== 'structured'" class="button small" :disabled="!active || stale || !canLaunchPart" @click="task(() => launchPart(true))">Launch part</button><button v-if="project.mode !== 'structured'" class="button small" :disabled="!active || stale || !canLaunchPart" @click="task(() => launchPart(false))">Stop part</button><output class="mode-pill" aria-label="Part playback status">{{ partStatus }}</output></div><p v-if="notice" class="feature-note" role="status">{{notice}}</p><ScoreWorkspace :key="project.id" :project="project" :user-id="user?.id||''" :members="members" :beats="scoreBeats" :editable="editable&&!active" :saving="saving" :save="saveScoreProject" @focus="selectedPart=$event" /></div>
