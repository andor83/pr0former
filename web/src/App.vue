...
function addNode(d: Descriptor, placement?: {x:number;y:number}) {
  ...
  const n: GraphNode = { parent: graphParent.value, id: newId(), kind: d.kind, label: d.label, x: pos.x, y: pos.y, channels: 2, parameters: Object.fromEntries(d.parameters.map(p => [p.id, p.default])) }
