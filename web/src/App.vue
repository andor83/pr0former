...
async function saveScoreProject(next:Project) {
  if(!project.value||saving.value||stage.value||!editable.value)throw new Error('Score editing is currently unavailable.')
  saving.value=true
  try {
...
async function downloadXML() { await scoreWorkspace.value?.flush(); if (!part.value || !project.value) return; const warnings=musicXMLExportWarnings(project.value); notice.value=warnings.join(' '); const url = URL.createObjectURL(new Blob([exportScoreMusicXML(project.value)], { type: 'application/xml' })); const link = document.createElement('a'); link.href = url; link.download = `${project.value.name}.musicxml`; link.click(); URL.revokeObjectURL(url) }
async function uploadXML(event: Event) { const input = event.target as HTMLInputElement; const file = input.files?.[0]; if (!file || !project.value) return; await scoreWorkspace.value?.flush(); await task(async () => { const result = importMusicXML(await file.text()); if (result.warnings.length && !window.confirm(`Import with these limitations?\n${result.warnings.join('\n')}`)) return; const next = clone(project.value!); next.parts = result.parts; next.score = result.score; for (const node of next.graph.nodes) if (node.part_id && !next.parts.some(part => part.id === node.part_id)) node.part_id = null; next.beats_per_bar = result.beatsPerBar; next.beat_unit = result.beatUnit; await saveProject(next) }); input.value = '' }
...
async function enterStage() { if (conductor.value) await transport('performance'); stage.value = true; selectedNode.value = null; projectPicker.value = false; pendingPort.value = null; selectedEdges.value = [] }
...
async function signOut() { await api('/logout', 'POST'); stage.value = false; browserOpen.value=false; systemOpen.value=false; user.value = null; project.value = null; socket?.close(); clearTimeout(reconnect) }
...
function keydown(event: KeyboardEvent) {
  ...
  if (event.defaultPrevented || event.isComposing || nodeMenu.value || savingSubgraph.value || selectedNode.value || creating.value || settingsOpen.value || systemOpen.value || consoleOpen.value || gearOpen.value || progress.value || browserOpen.value || quickNodes.value || consoleProject) return
