...
<template>
  <section class="browser-input-picker">
    <label>Machine name<input aria-label="Machine name" :value="localMachineName" maxlength="80" :disabled="disabled || browserInputBusy[inputKey]" @change="saveMachineName(($event.target as HTMLInputElement).value)"></label>
    <label><span class="field-title">Local audio input device<HelpNote label="Local audio input device">Device selection applies to this browser or app session. The node’s microphone button mutes or unmutes its graph output. Capture connects automatically when the engine is enabled for your assigned input. Monitor shows the connection and lets you stop capture. Muting keeps the connection open.</HelpNote></span><select v-model="choice" aria-label="Local audio input device" :disabled="disabled || asking || browserInputBusy[inputKey]">
    ...
    <button class="button small" :disabled="disabled || asking" @click="refreshBrowserInputs">Refresh local audio inputs</button>
    <button class="button small" :disabled="disabled || asking || browserInputBusy[inputKey]" @click="permit">{{asking ? 'Waiting for permission…' : 'Grant microphone access'}}</button>
