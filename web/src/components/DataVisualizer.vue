...
<div class="data-visualizer" :class="{compact,stale}">
  ...
  <template v-else><p v-if="!data?.channels" class="visualizer-placeholder">Activate the show to visualize this signal.</p><div v-else class="visualizer-channels"><section v-for="(_,index) in data.channels" :key="index"><header>CH {{index+1}} <span v-if="stale">STALE</span><span v-else-if="!data.ready">FILLING FFT</span></header><canvas :ref="el=>setCanvas(el,index)" role="img" :aria-label="`Channel ${index+1} ${spectral?'spectrogram, FFT bins and phase':'spectrogram'}`"/><div v-if="spectral" class="spectrum-key"><span>Magnitude (dB) · bins 0–{{data.size! / 2}}</span><span>Phase · −π…π rad</span></div></section></div><small>One history column per {{blockSize}}-frame DSP block · 20 Hz display<span v-if="spectral && data"> · {{data.polar?'Polar':'Cartesian'}} frames</span></small></template>
</div>
</template>
