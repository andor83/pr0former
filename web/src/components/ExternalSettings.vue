...
<h3 aria-label="Server MIDI devices">Server MIDI devices<HelpNote label="Server MIDI devices">Devices available to this performance server, including OS virtual MIDI ports.</HelpNote></h3>
...
<h4 aria-label="MIDI inputs">MIDI inputs<HelpNote label="MIDI inputs">Audio/MIDI devices listed here belong to the server. Add a MIDI input node and select its physical port in the node modal to route incoming messages. For a keyboard attached to this client, use Local MIDI input and choose its device in that node’s modal.</HelpNote></h4><p v-for="name in devices?.midi_inputs" :key="name">{{name}}</p><p v-if="!devices?.midi_inputs?.length">No MIDI inputs detected on the server.</p>

<h4 aria-label="MIDI outputs">MIDI outputs<HelpNote label="MIDI outputs">Select a server MIDI output port on a MIDI output node or score part to send to an external instrument. Bring that instrument’s audio back through an Audio input if it should join the graph mix.</HelpNote></h4><p v-for="name in devices?.midi_outputs" :key="name">{{name}}</p><p v-if="!devices?.midi_outputs?.length">No MIDI outputs detected on the server.</p>

</template>
<template v-else>
<h3 aria-label="OSC network bindings">OSC network bindings<HelpNote label="OSC network bindings">Network bindings apply to every project on this server. Incoming messages affect only the active show.</HelpNote></h3>
...
<label class="check-label"><input v-model="osc.receive_enabled" type="checkbox">Receive OSC</label><HelpNote label="Receive OSC">Enabling reception allows devices on the selected networks to control the active show without a browser login.</HelpNote>
...
<label><span class="field-title">OSC source port<HelpNote label="OSC source port">Source port 0 lets the OS choose a port. The destination port belongs to each part below.</HelpNote></span><input aria-label="OSC source port" v-model.number="osc.send_port" type="number" min="0" max="65535" step="1"></label>
...
<details><summary>Incoming OSC addresses<HelpNote>Use the node ID listed below or in exported project JSON. Incoming values are transient and do not change saved literals or undo history. UDP delivery is best effort.</HelpNote></summary><p>Send individual UDP OSC messages; bundles and timetags are not supported.</p><ul><li><code>/pr0former/play</code>, <code>/pr0former/pause</code>, <code>/pr0former/stop</code>: no arguments.</li><li><code>/pr0former/tempo</code>: one number, 1–400 BPM. Running changes wait for the next beat. A connected global clock tempo input takes precedence.</li><li><code>/pr0former/node/{id}/control</code>: one number or string matching an unconnected graphical control’s mode and limits.</li><li><code>/pr0former/node/{id}/bang</code>: no arguments; triggers an unconnected Bang control.</li></ul><p v-for="n in project.graph.nodes.filter(n=>n.kind==='control_input')" :key="n.id">{{n.label}}: <code>{{n.id}}</code></p></details>
</template>
...
<label><span class="field-title">OSC address<HelpNote label="OSC address">Score output sends two integers: pitch (0–127), velocity (0–127; zero releases the note). Changes save to this project.</HelpNote></span><input aria-label="OSC address" :value="part.osc_address" placeholder="/pr0former/note" @change="update('osc_address',($event.target as HTMLInputElement).value)"></label>
