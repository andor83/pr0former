...
<template>
  <section class="local-midi-picker">
    ...
    <label>Local MIDI device<select v-model="state.device" aria-label="Local MIDI device" :disabled="!editable||state.connected||state.connecting"><option value="">Select a device…</option><option v-for="device in midi.devices.value" :key="device.id" :value="device.id">{{device.name}}</option></select></label>
    ...
    <HelpNote>Connect this node's MIDI outlet to Knobs or Sliders, then click a control to learn its MIDI assignment. Escape or clicking away cancels learning. Device selection stays in this session; MIDI continues while the modal is closed.</HelpNote>
  </section>
</template>
