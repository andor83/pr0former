...
<section class="parameter-row looper-status" aria-label="Loop tracks">
  <h3 aria-label="Loop tracks">Loop tracks<HelpNote label="Loop tracks">Send track numbers 1–8; return the command input to 0 before repeating that track. Freeform starts immediately. Beat mode waits for the next bar of the running graph clock. Stops are immediate.<br /><br />Stopping recording does not automatically play it. Start playback to loop the captured audio. Recording again replaces that track. Completed recordings are saved on this server per project and node. Disabling the engine finishes and saves ongoing recordings. Clear stops the track and deletes its saved audio. Saved tracks load ready to play when the engine starts.</HelpNote></h3>
