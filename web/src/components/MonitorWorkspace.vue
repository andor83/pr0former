...
import type { GraphNode, Telemetry } from '../types'
import { api } from '../api'
import MonitorPanel from './MonitorPanel.vue'
import LevelMeter from './LevelMeter.vue'
const props = defineProps<{projectId:string;nodes:GraphNode[];active:boolean;stale:boolean;telemetry:Telemetry|null;age:number;sampleRate:number;blockSize:number;visible:boolean;stage:boolean}>()
const inputs = computed(() => props.nodes.filter(n=>['input','browser_input'].includes(n.kind)))
const outputs = computed(() => props.nodes.filter(n=>['output','monitor_output'].includes(n.kind)))
...
async function refresh() {
  ...
  try { const value=await api<NonNullable<typeof resources.value>>('/system/stats');if(current===generation){resources.value=value;resourceError.value=''} }
  catch {if(current===generation){resources.value=undefined;resourceError.value='Resource stats unavailable'}}
}
...
<template>
  <section class="monitor-workspace" :class="{'stage-monitor':stage}" aria-label="Performance monitor">
    ...
    <div v-show="!stage" class="meter-banks">
      <section v-for="bank in [{title:'Inputs',nodes:inputs},{title:'Outputs',nodes:outputs}]" :key="bank.title" class="meter-bank" :aria-label="`${bank.title} VU meters`">
        <header><h2>{{bank.title}}<span>{{bank.nodes.length}}</span></h2><span class="bank-caption">SAMPLE PEAK · dBFS</span></header>
        <div v-if="bank.nodes.length" class="meter-bank-body">
          <div class="vu-scale" aria-hidden="true"><span v-for="db in [0,-12,-24,-36,-48,-60]" :key="db">{{db}}</span></div>
          <div class="vu-strips"><LevelMeter v-for="node in bank.nodes" :key="node.id" :node="node" :peak="telemetry?.levels?.[node.id]" :stale="!active || stale || telemetry?.levels?.[node.id] === undefined" /></div>
        </div>
        <div v-else class="meters-empty"><Headphones :size="25" /><p>No {{bank.title.toLowerCase()}} in this graph</p><span>Add {{bank.title==='Inputs' ? 'an Audio input or Browser input' : 'an Audio output or Monitor output'}} node to see its level.</span></div>
      </section>
    </div>
  </section>
</template>
