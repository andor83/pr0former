...
defineExpose({ toggleMonitor: () => monitorPanel.value?.toggle(), monitorState, monitorError })
...
<template>
  <section class="monitor-workspace" :class="{'stage-monitor':stage}" aria-label="Performance monitor">
    <aside class="monitor-sidebar" aria-label="Monitor controls and statistics">
      ...
      <MonitorPanel ref="monitorPanel" :project-id="projectId" :active="active" :nodes="nodes" compact />
