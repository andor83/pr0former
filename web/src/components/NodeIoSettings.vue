...
<template>
  <section class="parameter-row node-io-settings">
    ...
    <template v-else><label :for="`osc-address-${node.id}`">OSC address</label><input :id="`osc-address-${node.id}`" v-model="draft.address" placeholder="/note" :disabled="disabled" @change="change"><template v-if="!input"><label :for="`osc-destination-${node.id}`">OSC destination</label><input :id="`osc-destination-${node.id}`" v-model="draft.destination" placeholder="synth.local:9000" :disabled="disabled" @change="change"></template><HelpNote v-if="!input">Destination accepts a DNS hostname or IPv4 address followed by :port. Names resolve on the server; the destination must have an IPv4 address.</HelpNote><HelpNote v-if="node.kind==='osc_input'||node.kind==='osc_output'">Enable OSC {{input?'receiving':'sending'}} in System settings. Messages carry one argument: a number, or text when the node is in text mode.</HelpNote><HelpNote v-else>Enable OSC {{input?'receiving':'sending'}} in System settings. Note messages contain two integers: pitch and velocity; velocity 0 releases that pitch.</HelpNote></template>
