...
<template>
  <dialog ref="dialog" class="parameter-modal node-parameter-modal" :class="{'script-modal':node.kind==='js_control','field-modal':granular}" @cancel.prevent="emit('close')" @keydown="trap" @pointerdown="backdropDown" @click="backdropClick" aria-labelledby="node-modal-title">
    ...
    <GranularFieldSources v-if="granular && sourcesOpen" :node="node" :samples="samples || []" :edges="edges" :nodes="nodes" :values="values" :active="active" :stale="stale" :editable="editable" :saving="saving" @close="sourcesOpen = false" @sources="(choices, parameters) => emit('sources', choices, parameters)" @change="(key, value) => emit('change', key, value)" />
  </dialog>
</template>
