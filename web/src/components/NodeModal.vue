...
<template>
  <dialog ref="dialog" class="parameter-modal" @cancel.prevent="emit('close')" @keydown="trap" aria-labelledby="node-modal-title">
    ...
    <div class="parameter-list"><section class="parameter-row"><label for="node-name">Node name</label><input id="node-name" :value="node.label" maxlength="256" :disabled="!editable || saving" @change="emit('rename',($event.target as HTMLInputElement).value)"><button v-if="node.kind==='subgraph'" class="button" @click="emit('expand')">Open subgraph</button></section>
      <section v-if="/^(send|receive)_(control|audio|spectral)$/.test(node.kind)" class="parameter-row"><label>Target name<input aria-label="Target name" placeholder="e.g. main" maxlength="256" :value="links.target?(active&&!stale?routeTarget??'':''):typeof node.control_value==='string'?node.control_value:''" :disabled="!editable||saving||!!links.target" @change="emit('control',($event.target as HTMLInputElement).value)"></label><p v-if="links.target" class="feature-note">Connected target: {{sourceName(links.target)}} / {{links.target.source_port}}</p><p v-if="active&&!stale&&values?._route_error" class="field-error" role="alert">Route target or audio/spectral format is incompatible.</p><p v-if="active&&!stale&&node.kind.startsWith('receive_')" class="feature-note">{{values?._route_connected||0}} matching sender(s)</p><p class="feature-note">Matching names connect within this project, with one sample of routing delay. An empty name disconnects.</p></section>
      ...
      <section v-if="['sample', 'phase_vocoder', 'poly_sampler'].includes(node.kind)" class="parameter-row"><label>Project sample<select aria-label="Project sample" :value="node.parameters.asset||0" :disabled="!editable||saving" @change="samples?.find(s=>s.asset===Number(($event.target as HTMLSelectElement).value)) && emit('sample',samples!.find(s=>s.asset===Number(($event.target as HTMLSelectElement).value))!)"><option :value="0" disabled>Select a project sample</option><option v-for="sample in samples||[]" :key="sample.id" :value="sample.asset!">{{sample.name}} · {{sample.channels}} ch</option></select></label><label class="button">Import audio sample<input type="file" style="display:none" :disabled="!editable || saving" @change="($event.target as HTMLInputElement).files?.[0] && emit('upload', ($event.target as HTMLInputElement).files![0])"></label><p class="feature-note">FFmpeg-supported audio, up to 30 seconds and 8 channels. Import preserves channels and converts to WAV at the current engine rate.</p></section>
      ...
      <section v-for="p in descriptor.parameters.filter(p => !p.id.includes('_curve_') && !p.id.startsWith('route_') && !(node.kind==='control_input' && ['min','max'].includes(p.id) && [0,4].includes(node.parameters.mode??2)))" :key="p.id" class="parameter-row" :class="{ driven: links[p.id] }">
