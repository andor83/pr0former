...
import GranularFieldSources from './GranularFieldSources.vue'
...
import { CONTAINER_PALETTE } from '../containerPalette'
const props = defineProps<{ applyScript:(script:import('../types').ScriptConfig,nodeId:string,projectId:string)=>Promise<void>; scriptStatus?:import('../types').ScriptStatus; oscMessage?:[number,number|string]; routeTargets?:Record<string,string>;routeTarget?:string;samples?:SampleEntry[];librarySamples?:SampleEntry[];attachSample?:(id:string)=>Promise<SampleEntry>; inputError?:string|null; ioStatus?:{error:string|null;dropped:number}; parts?: Part[]; projectId?:string; visualization?:Visualization; sampleRate?:number;blockSize?:number; interfaces?: {id:number;name:string}[]; node: GraphNode; descriptor: Descriptor; edges: GraphEdge[]; nodes: GraphNode[]; values?: Record<string, number>; stale: boolean; editable: boolean; active: boolean; saving: boolean }>()
const emit = defineEmits<{ sampleChoices:[choices:import('../types').SampleChoice[]]; sources:[choices:import('../types').SampleChoice[],parameters:Record<string,number>]; sample:[sample:SampleEntry]; rename: [label:string]; part: [id:string]; io: [value:IoConfig]; expand: []; close: []; change: [key: string, value: number]; disconnect: [edge: GraphEdge]; source: [id: string]; undo: []; remove: []; upload: [file: File]; control: [value:number|string]; curve: [parameters: Record<string, number>]; channels: [width: number] }>()
...
const explanationDraft=ref(typeof props.node.control_value==='string'?props.node.control_value:'')
watch(()=>[props.node.id,props.node.control_value],()=>{explanationDraft.value=typeof props.node.control_value==='string'?props.node.control_value:''})
...
// Granular Field: the field pad owns x, y, focus and every per-source setting; the sources dialog edits the slot list.
const granular = computed(() => props.node.kind === 'granular_field')
const sourcesOpen = ref(false)
watch(() => props.node.id, () => { sourcesOpen.value = false })
const fieldTelemetry = computed(() => props.active && !props.stale ? props.values : undefined)
const fieldControl = computed(() => controlPoint(props.node, fieldTelemetry.value, Object.keys(links.value)))
const fieldFocus = computed(() => fieldValue(props.node, 'focus', fieldTelemetry.value))
const fieldPad = computed(() => {
  const sources = fieldSources(props.node, Object.keys(groupedLinks.value))
  const points = sources.map(s => sourcePoint(props.node, s, fieldTelemetry.value))
  const preview = fieldControl.value ? sourceWeights(fieldControl.value, points, fieldFocus.value) : points.map(() => 0)
  return sources.map((s, i) => ({ key: s.port, label: s.label, kind: s.kind, point: points[i]!, weight: fieldTelemetry.value ? props.values?.[s.weightKey] ?? 0 : preview[i]!, missing: !!fieldTelemetry.value && !!props.values?.[s.missingKey ?? ''], keys: { x: s.keys.x, y: s.keys.y } }))
})
function parameterLabel(p: Parameter) { return granular.value && p.id === 'pitch' && (props.node.parameters.randomize_pitch ?? 0) > 0 ? 'Pitch randomness (± semitones)' : p.label }
...
function nodeIsSliderRange(p:Parameter){return (props.node.kind==='sliders'&&['min','max','step'].includes(p.id))||(props.node.kind==='control_input'&&p.id==='step')}
// On/off parameters render as a checkbox beside their label instead of a slider.
function booleanParameter(p:Parameter){return (props.node.kind==='control_input'&&['hide_chrome','changes_only'].includes(p.id))||(props.node.kind==='js_control'&&p.id==='midi_passthru')||(props.node.kind==='granular_field'&&p.id==='randomize_pitch')||(props.node.kind==='lfo'&&p.id==='clock_lock')}
...
<template>
  <dialog ref="dialog" class="parameter-modal node-parameter-modal" :class="{'script-modal':node.kind==='js_control','field-modal':granular}" @cancel.prevent="emit('close')" @keydown="trap" @pointerdown="backdropDown" @click="backdropClick" aria-labelledby="node-modal-title">
    ...
    <div class="parameter-list"><section class="parameter-row"><label for="node-name">Node name</label><input id="node-name" v-model="nameDraft" maxlength="256" :disabled="!editable || saving" @change="emit('rename',($event.target as HTMLInputElement).value)"><button v-if="node.kind==='subgraph'" class="button" @click="emit('expand')">Open subgraph</button></section>
      <section v-if="granular" class="parameter-row field-section">
        <div class="parameter-heading"><label><span class="field-title">Field<HelpNote label="Field">Sources sit on a two-dimensional field and the control point roams it: the closer the point is to a source, the more grains come from it, weighted by exp(-(distance/focus)^2). Drag the control point or any source circle, or focus a handle and use the arrow keys (Shift for larger steps). Cabled axes are locked and follow their cables. Circles grow with the share of the cloud they supply. Live inputs appear only while their audio port is cabled.</HelpNote></span></label><button class="button small" :disabled="!editable || saving" @click="sourcesOpen = true">Edit sources</button></div>
        <div class="field-layout">
          <GranularFieldPad :sources="fieldPad" :control="fieldControl" :focus="fieldFocus" :locked="Object.keys(links)" :editable="editable && !saving" :live="!!fieldTelemetry" @change="(key, value) => emit('change', key, value)" />
          <div class="field-readouts">
            <template v-for="p in descriptor.parameters.filter(p => ['x', 'y', 'focus'].includes(p.id))" :key="p.id">
              <label class="field-readout" :class="{ driven: links[p.id] }"><span>{{ p.label }}</span>
                <template v-if="links[p.id]"><output>{{ active && !stale ? values?.[p.id] === undefined ? '—' : formatSlider(values[p.id]!, false) : '—' }}</output><button class="text-button" @click="emit('source', links[p.id].source)">{{ sourceName(links[p.id]) }} / {{ links[p.id].source_port }}</button></template>
                <input v-else type="number" :min="p.min" :max="p.max" step="0.01" :aria-label="p.label" :value="textDraft[p.id] ?? formatSlider(value(p), false)" :disabled="!editable || saving" @input="textDraft[p.id] = ($event.target as HTMLInputElement).value" @change="edit(p, ($event.target as HTMLInputElement).value, true)">
              </label>
            </template>
            <p v-if="!fieldPad.length" class="feature-note">No sources yet. Use Edit sources to add project samples, or cable audio into a live input.</p>
            <p v-else-if="active && !stale" class="feature-note">{{ values?._grains ?? 0 }} grains sounding</p>
          </div>
        </div>
      </section>
      ...
      <section v-if="node.kind==='container'" class="parameter-row"><div class="parameter-heading"><label id="container-color-label">Color</label><span class="small-tag">STRUCTURAL</span></div><div class="container-palette" role="radiogroup" aria-labelledby="container-color-label"><button v-for="(swatch,index) in CONTAINER_PALETTE" :key="swatch.hex" type="button" role="radio" class="container-swatch" :class="{active:Math.round(node.parameters.color??0)===index}" :aria-checked="Math.round(node.parameters.color??0)===index" :aria-label="swatch.name" :title="swatch.name" :style="{'--swatch':swatch.hex}" :disabled="!editable||saving" @click="emit('change','color',index)"></button></div></section>
      <section v-if="node.kind==='container'" class="parameter-row"><label><span class="field-title">Explanation<HelpNote label="Explanation">Shown in the info hover beside the container's title on the graph, and inline when descriptions are expanded. Up to 2,000 characters.</HelpNote></span><textarea v-model="explanationDraft" rows="5" maxlength="2000" :disabled="!editable||saving" aria-label="Container explanation" placeholder="What do the nodes in this group do?" @change="emit('control',explanationDraft.trim())"></textarea></label></section>
      ...
      <section v-if="node.kind !== 'record' && node.kind !== 'pitch_tracker' && node.kind !== 'subgraph' && node.kind !== 'container' && !node.kind.endsWith('_control') && descriptor.category !== 'Math' && descriptor.category !== 'Control' && descriptor.category !== 'Timing'" class="parameter-row"><div class="parameter-heading"><label for="node-channels">Audio channels</label><span class="small-tag">STRUCTURAL</span></div><select id="node-channels" :value="node.channels" :disabled="!editable || saving" @change="emit('channels', +($event.target as HTMLSelectElement).value)"><option v-for="width in 8" :key="width" :value="width">{{ width }} channel{{ width === 1 ? '' : 's' }}</option></select></section>
      ...
      <SampleSelectorOptions v-if="node.kind==='sample_selector'" :choices="node.sample_choices||[]" :samples="samples||[]" :library="librarySamples||[]" :attach="attachSample" :disabled="!editable||saving" @change="emit('sampleChoices',$event)" />
      <section v-if="sampleBacked" class="parameter-row"><label>Project sample<SuggestInput label="Project sample" :value="sampleValue" :suggestions="(samples||[]).map(s=>({value:s.name,detail:`${s.asset} · ${s.channels} ch`}))" :disabled="!editable||saving||!!links.sample_id" placeholder="Search samples or enter numeric ID" @change="chooseSample" /></label><label class="button"><span class="field-title">Import audio sample<HelpNote label="Import audio sample">Search by sample name or enter a numeric sample ID. Imported audio is converted to WAV at the current engine rate.</HelpNote></span><input aria-label="Import audio sample" type="file" style="display:none" :disabled="!editable || saving || !!links.sample_id" @change="($event.target as HTMLInputElement).files?.[0] && emit('upload', ($event.target as HTMLInputElement).files![0])"></label></section>
      ...
      <section v-for="p in descriptor.parameters.filter(p => !(['knobs','sliders'].includes(node.kind) && /^(channel|controller)_/.test(p.id) && Number(p.id.split('_').at(-1))>(node.parameters.count??4)) && !(sampleBacked && p.id === 'asset') && !p.id.includes('_curve_') && !p.id.startsWith('route_') && !(['input','output'].includes(node.kind) && p.id === 'interface') && !(node.kind==='control_input' && ['min','max','step'].includes(p.id) && [0,4].includes(node.parameters.mode??2)) && !(node.kind==='container' && ['color','width','height'].includes(p.id)) && !(granular && (['x','y','focus'].includes(p.id) || (/^sample_\d+$/.test(p.id) && !links[p.id]))))" :key="p.id" class="parameter-row" :class="{ driven: links[p.id], 'boolean-row': booleanParameter(p) }">
        <div class="parameter-heading"><label :for="`param-${p.id}`">{{ parameterLabel(p) }}</label><input v-if="booleanParameter(p)" :id="`param-${p.id}`" type="checkbox" class="heading-checkbox" :aria-label="p.label" :checked="(node.parameters[p.id]??p.default)===1" :disabled="!editable||saving" @change="emit('change',p.id,($event.target as HTMLInputElement).checked?1:0)"><HelpNote v-if="node.kind==='pitch_tracker' && p.id==='slots'" label="Pitch slots">Reducing slots disconnects removed outputs. Empty slots output −1.</HelpNote><HelpNote v-if="node.kind==='js_control' && p.id==='midi_passthru'" label="MIDI passthrough">Relays every incoming MIDI message to the MIDI output unchanged while the script still receives it; MIDI the script sends is merged into the same stream. Turn off to let the script consume incoming MIDI and forward only what it sends.</HelpNote><HelpNote v-if="node.kind==='control_input' && p.id==='step'" label="Step">Snaps slider dragging and the arrow keys and buttons to this increment. 0 keeps the defaults: 1 for Integer and Float, 0.01 for Slider.</HelpNote><span v-if="p.structural" class="small-tag">STRUCTURAL</span><span v-else-if="links[p.id]" class="connected-label"><Link2 :size="12" /> CONNECTED</span></div>
        ...
        <template v-else-if="['sin','cos'].includes(node.kind) && p.id==='units' && !links[p.id]"><select aria-label="Input units" :value="node.parameters.units??0" :disabled="!editable||saving" @change="emit('change','units',Number(($event.target as HTMLSelectElement).value))"><option v-for="(label,index) in ['Radians','Degrees','Normalized · 1 is one turn']" :key="label" :value="index">{{label}}</option></select></template>
        <template v-else-if="node.kind==='smooth_change' && p.id==='curve' && !links[p.id]"><select aria-label="Curve" :value="node.parameters.curve??3" :disabled="!editable||saving" @change="emit('change','curve',Number(($event.target as HTMLSelectElement).value))"><option v-for="(label,index) in ['Linear','Ease in','Ease out','Ease in–out','Exponential · one-pole chase','Spring · critically damped']" :key="label" :value="index">{{label}}</option></select></template>
        ...
        <template v-else-if="booleanParameter(p)"></template>
        <template v-else-if="node.kind==='control_input' && p.id==='step'"><div class="parameter-controls"><div class="number-field"><input :id="`param-${p.id}`" type="number" min="0" step="0.01" :value="textDraft[p.id] ?? formatSlider(value(p))" :disabled="!editable||saving" :aria-label="p.label" @input="textDraft[p.id] = ($event.target as HTMLInputElement).value" @change="edit(p, ($event.target as HTMLInputElement).value, true)"><span>0 = auto</span></div></div><p v-if="errors[p.id]" class="field-error">{{ errors[p.id] }}</p></template>
    ...
    <GranularFieldSources v-if="granular && sourcesOpen" :node="node" :samples="samples || []" :library="librarySamples || []" :attach="attachSample" :edges="edges" :nodes="nodes" :values="values" :active="active" :stale="stale" :editable="editable" :saving="saving" @close="sourcesOpen = false" @sources="(choices, parameters) => emit('sources', choices, parameters)" @change="(key, value) => emit('change', key, value)" />
  </dialog>
</template>
