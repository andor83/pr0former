...
const props = defineProps<{ projectId?:string; visualization?:Visualization; sampleRate?:number;blockSize?:number; interfaces?: {id:number;name:string}[]; node: GraphNode; descriptor: Descriptor; edges: GraphEdge[]; nodes: GraphNode[]; values?: Record<string, number>; stale: boolean; editable: boolean; active: boolean; saving: boolean }>()
const emit = defineEmits<{ rename: [label:string]; expand: []; close: []; change: [key: string, value: number]; disconnect: [edge: GraphEdge]; source: [id: string]; undo: []; remove: []; upload: [file: File]; control: [value:number|string]; curve: [parameters: Record<string, number>]; channels: [width: number] }>()
...
<template>
  <dialog ref="dialog" class="parameter-modal" @cancel.prevent="emit('close')" @keydown="trap" aria-labelledby="node-modal-title">
    ...
    <div class="parameter-list"><section class="parameter-row"><label for="node-name">Node name</label><input id="node-name" :value="node.label" maxlength="256" :disabled="!editable || saving" @change="emit('rename',($event.target as HTMLInputElement).value)"><button v-if="node.kind==='subgraph'" class="button" @click="emit('expand')">Open subgraph</button></section>
      ...
      <section v-if="node.kind !== 'subgraph' && !node.kind.endsWith('_control') && descriptor.category !== 'Math' && descriptor.category !== 'Control' && descriptor.category !== 'Timing'" class="parameter-row"><div class="parameter-heading"><label for="node-channels">Audio channels</label><span class="small-tag">STRUCTURAL</span></div><select id="node-channels" :value="node.channels" :disabled="!editable || saving" @change="emit('channels', +($event.target as HTMLSelectElement).value)"><option v-for="width in 8" :key="width" :value="width">{{ width }} channel{{ width === 1 ? '' : 's' }}</option></select></section>
      ...
      <div v-if="!descriptor.parameters.length && !['browser_input','clock','subgraph'].includes(node.kind) && !node.kind.startsWith('subgraph_') && !node.kind.endsWith('_visualizer')" class="empty-parameters"><Activity :size="24" /><p>This node has no editable parameters.</p><p>Output: {{ formatValue(values?._out) }}</p></div>
    </div>
