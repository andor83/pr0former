...
<template>
  <dialog ref="dialog" class="parameter-modal node-parameter-modal" @cancel.prevent="emit('close')" @keydown="trap" @pointerdown="backdropDown" @click="backdropClick" aria-labelledby="node-modal-title">
    <header class="modal-header"><div class="modal-icon">{{ descriptor.symbol }}</div><div><div class="eyebrow">{{ descriptor.category }} / {{ ['record','pitch_tracker'].includes(node.kind)?'AUTO':node.channels }} CHANNELS</div><h2 id="node-modal-title">{{ node.label }}</h2><small v-if="node.label.trim().toLowerCase()!==descriptor.label.trim().toLowerCase()" class="node-kind">{{ descriptor.label }}</small></div><button class="icon-button modal-close" aria-label="Close parameters" @click="emit('close')"><X :size="20" /></button></header>
    <HelpNote v-if="node.library">Library version {{node.library.version}} · embedded copy; local edits do not change the library.</HelpNote><HelpNote variant="modal-description">{{ descriptor.description }}</HelpNote>
    ...
    <div class="parameter-list"><section class="parameter-row"><label for="node-name">Node name</label><input id="node-name" v-model="nameDraft" maxlength="256" :disabled="!editable || saving" @change="emit('rename',($event.target as HTMLInputElement).value)"><button v-if="node.kind==='subgraph'" class="button" @click="emit('expand')">Open subgraph</button></section>
      <section v-if="/^(send|receive)_(control|audio|spectral)$/.test(node.kind)" class="parameter-row"><label>Target name<input aria-label="Target name" placeholder="e.g. main" maxlength="256" list="route-target-suggestions" autocomplete="off" :value="links.target?(active&&!stale?routeTarget??'':''):typeof controlDraft==='string'?controlDraft:''" :disabled="!editable||saving||!!links.target" @input="controlDraft=($event.target as HTMLInputElement).value" @change="!links.target&&emit('control',controlDraft)"></label><datalist id="route-target-suggestions"><option v-for="target in targetSuggestions" :key="target" :value="target"/></datalist><template v-if="links.target"><HelpNote>Target is controlled by a connection. Disconnect it to enter a target manually.</HelpNote><div v-for="edge in groupedLinks.target" :key="edge.id" class="driver-source"><button @click="emit('source',edge.source)"><ArrowUpRight :size="14"/>{{sourceName(edge)}} / {{edge.source_port}}<small v-if="groupedLinks.target.length>1&&links.target.id===edge.id"> · current</small></button><button :disabled="!editable||saving" :aria-label="`Disconnect ${sourceName(edge)} from Target name`" @click="emit('disconnect',edge)"><Unplug :size="14"/> Disconnect</button></div></template><HelpNote v-else>Type a new target or choose an existing {{node.kind.split('_')[1]}} target in this project. Names match exactly, including case.</HelpNote><p v-if="active&&!stale&&values?._route_error" class="field-error" role="alert">Route target or audio/spectral format is incompatible.</p><p v-if="active&&!stale&&node.kind.startsWith('receive_')" class="feature-note">{{values?._route_connected||0}} matching sender(s)</p><HelpNote>Matching names connect within this project, with one sample of routing delay. An empty name disconnects.</HelpNote></section>
      <section v-if="node.kind==='meter'" class="parameter-row"><HelpNote>The node draws one dBFS meter per channel, matching the monitor tab. Each channel's Level output carries its amplitude as a control value (0–1 peak, instant rise, falling over the fall time); reducing the channel count removes the upper Level outputs and their cables.</HelpNote></section>
      <section v-if="node.kind==='pitch_tracker'" class="parameter-row"><HelpNote>All incoming audio channels are averaged to mono. Pitches rank strongest first; empty slots output −1. Analysis window: {{((node.parameters.fft_size??8192)/(sampleRate||48000)*1000).toFixed(1)}} ms, updated every {{((node.parameters.fft_size??8192)/(sampleRate||48000)*250).toFixed(1)}} ms.</HelpNote></section>
      ...
      <section v-if="node.kind==='record'" class="parameter-row" aria-label="Recording status">
        ...
        <HelpNote>The node name above is the recording filename prefix. Each take gets a timestamp and is saved in this server’s recordings folder, owned by this project. Channels follow the audio connection automatically.</HelpNote>
        <HelpNote>Send a positive pulse to Start or Stop; return to zero to rearm. Stop or disable the engine to finalize the WAV. Pausing the show leaves recording running. Large takes split into 1 GiB segments. Download browsing will be added later.</HelpNote>
      </section>
      ...
      <section v-if="node.kind==='control_visualizer'" class="parameter-row"><template v-if="links.in"><p class="connected-label">CONNECTED · input is read-only</p><button class="text-button" @click="emit('source',links.in.source)">{{sourceName(links.in)}} / {{links.in.source_port}}</button></template><template v-else><label>Disconnected input type<select aria-label="Disconnected input type" :value="typeof node.control_value==='string'?'text':'number'" :disabled="!editable||saving" @change="emit('control',($event.target as HTMLSelectElement).value==='text'?'':0)"><option value="number">Number</option><option value="text">String</option></select></label><label>Disconnected input value<input aria-label="Disconnected input value" :type="typeof node.control_value==='string'?'text':'number'" v-model="controlDraft" :disabled="!editable||saving" @change="emit('control',controlDraft)"></label><HelpNote>Used only with no input connection. Strings support up to 256 UTF-8 bytes.</HelpNote></template></section>
      <section v-if="node.kind==='adsr'" class="parameter-row"><div class="parameter-heading"><label>Envelope</label><span class="small-tag">DRAG THE HANDLES</span></div><EnvelopeGraph :envelope="{attack:(links.attack&&active&&!stale?values?.attack:undefined)??node.parameters.attack??10,decay:(links.decay&&active&&!stale?values?.decay:undefined)??node.parameters.decay??100,sustain:(links.sustain&&active&&!stale?values?.sustain:undefined)??node.parameters.sustain??0.7,release:(links.release&&active&&!stale?values?.release:undefined)??node.parameters.release??200}" :level="active&&!stale?values?._out:undefined" :gate="active&&!stale&&(values?.gate??0)>0" :editable="editable" :locked="Object.keys(links)" @change="(key,value)=>emit('change',key,value)" /><HelpNote>Drag the attack, decay/sustain and release handles, or focus a handle and use the arrow keys (Shift for larger steps). The time axis is logarithmic per stage, so short attacks and long releases are both visible, and the sustain plateau absorbs whatever width is left; the Zoom slider stretches or compresses the axis while everything stays in view. The release ends at the right edge, so drag its handle left to lengthen it. Drags stop where the plateau would vanish; zoom out to go further. Connected parameters are locked; the dotted line is the live output level.</HelpNote><HelpNote>Every input is edge triggered: a rising gate or retrigger pulse attacks, a falling gate or rising note_off pulse releases (even with Gate left at 1). For pulse sources (Piano, Part MIDI, MIDI to control) wire trigger to retrigger and note_off to note_off; for held sources wire gate to Gate.</HelpNote></section>
