...
const emit = defineEmits<{ close: []; change: [key: string, value: number]; disconnect: [edge: GraphEdge]; source: [id: string]; undo: []; remove: []; upload: [file: File]; control: [value:number|string]; channels: [width: number] }>()
  ...
  <dialog ref="dialog" class="parameter-modal" @cancel.prevent="emit('close')" @keydown="trap" aria-labelledby="node-modal-title">
    ...
    <div class="parameter-list">
      ...
      <section v-for="p in descriptor.parameters" :key="p.id" class="parameter-row" :class="{ driven: links[p.id] }">
        ...
        <template v-if="node.kind.endsWith('_visualizer') && ['size','overlap'].includes(p.id)"><select :aria-label="p.label" :value="node.parameters[p.id]??p.default" :disabled="active||!editable||saving" @change="emit('change',p.id,+($event.target as HTMLSelectElement).value)"><option v-for="n in p.id==='size'?[256,512,1024,2048,4096,8192]:[2,4]" :key="n" :value="n">{{n}}</option></select></template>
        <template v-else-if="p.id==='interface' && !links[p.id]"><select aria-label="Audio interface" :value="node.parameters.interface || 0" :disabled="!editable||active||saving" @change="emit('change','interface',+($event.target as HTMLSelectElement).value)"><option :value="0">All enabled interfaces</option><option v-for="i in interfaces" :key="i.id" :value="i.id">{{i.name}}</option><option v-if="node.parameters.interface && !interfaces?.some(i=>i.id===node.parameters.interface)" :value="node.parameters.interface" disabled>Unavailable interface — select another</option></select></template>
