...
<template>
  <dialog ref="dialog" class="parameter-modal node-parameter-modal" @cancel.prevent="emit('close')" @keydown="trap" aria-labelledby="node-modal-title">
    ...
    <div class="parameter-list"><section class="parameter-row"><label for="node-name">Node name</label><input id="node-name" v-model="nameDraft" maxlength="256" :disabled="!editable || saving" @change="emit('rename',($event.target as HTMLInputElement).value)"><button v-if="node.kind==='subgraph'" class="button" @click="emit('expand')">Open subgraph</button></section>
      ...
      <section v-if="node.kind==='meter'" class="parameter-row"><p class="feature-note">The node draws one dBFS meter per channel, matching the monitor tab. Each channel's Level output carries its amplitude as a control value (0–1 peak, instant rise, falling over the fall time); reducing the channel count removes the upper Level outputs and their cables.</p></section>
      ...
      <section v-for="p in descriptor.parameters.filter(p => !(sampleBacked && p.id === 'asset') && !p.id.includes('_curve_') && !p.id.startsWith('route_') && !(['input','output'].includes(node.kind) && p.id === 'interface') && !(node.kind==='control_input' && ['min','max'].includes(p.id) && [0,4].includes(node.parameters.mode??2)))" :key="p.id" class="parameter-row" :class="{ driven: links[p.id] }">
