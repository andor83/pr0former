...
<template>
  ...
  <div v-else class="patch-node" :class="[signal, { selected, 'math-node': isMath, 'visualizer-node':visualizer }]" :style="{ minHeight: `${height}px`, width: data.node.kind==='pitch_tracker' ? `${Math.max(240,24+92*(data.node.parameters.slots??1))}px` : data.node.kind==='piano' ? `${24+256*Math.min(data.node.parameters.octaves??1,10-(data.node.parameters.octave??4))}px` : undefined }" tabindex="0" @mousedown="controlSelect" @click="event => { if(event.ctrlKey) event.stopPropagation() }" @contextmenu.prevent.stop="!$event.ctrlKey && data.contextMenu(id, $event)" @keydown.enter.stop.prevent="data.open(id)" @dblclick.stop="data.open(id)">
