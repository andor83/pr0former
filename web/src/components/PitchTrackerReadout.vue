...
const emit=defineEmits<{connect:[port:string]}>()
...
<template>
  <div class="tracker-pitches" aria-label="Detected pitches, strongest first">
    <div v-for="slot in slots" :key="slot" class="tracker-slot">
      ...
      <Handle :id="`pitch${slot}`" type="source" :position="Position.Bottom" class="control" :aria-label="`Pitch ${slot} output`" @click.stop="emit('connect',`pitch${slot}`)" />
    </div>
  </div>
</template>
<style scoped>
.tracker-pitches{position:absolute;bottom:0;left:12px;right:12px;display:flex;height:76px;gap:8px}.tracker-slot{position:relative;flex:1;text-align:center;border-top:1px solid var(--line);padding-top:7px}.tracker-slot small{font-size:9px;color:var(--muted);display:block}.tracker-slot output{display:block;font-size:21px;color:var(--amber);font-variant-numeric:tabular-nums}.tracker-slot span{font-size:9px;color:var(--muted)}.tracker-slot>.vue-flow__handle{top:auto;bottom:0;left:50%;transform:translate(-50%,50%) rotate(45deg)}
</style>
