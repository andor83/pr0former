...
function keys(e:KeyboardEvent){
 ...
 if(e.key==='Tab'){const items=(e.currentTarget as HTMLElement).querySelectorAll<HTMLElement>('input,button,[tabindex="0"]');const first=items[0],last=items[items.length-1];if(e.shiftKey&&document.activeElement===first){e.preventDefault();last?.focus()}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first?.focus()}}
}
let cleanup=()=>{}
