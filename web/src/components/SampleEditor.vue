...
<div class="parameter-list">
  ...
  <label><span class="field-title">Root pitch<HelpNote label="Root pitch">Sets the root MIDI note when this sample is assigned to a pitched sampler. Existing sampler settings and connected root-note controls are preserved.</HelpNote></span><select aria-label="Root pitch" :value="draft.root_note??''" :disabled="!sample.can_edit||busy" @change="draft.root_note=($event.target as HTMLSelectElement).value===''?null:Number(($event.target as HTMLSelectElement).value)"><option value="">Not set</option><option v-for="note in 128" :key="note-1" :value="note-1">{{midiNoteLabel(note-1)}}</option></select></label>

  <label class="check-label"><input aria-label="Global — available to every user and project" v-model="draft.global" type="checkbox" :disabled="!sample.can_publish||busy">Global — available to every user and project<HelpNote label="Global — available to every user and project">Project members can access samples already added to their project. Turning Global off hides the sample from future browsing; existing project copies remain available. Once shared globally, deletion requires an administrator, even after unsharing.</HelpNote></label>
...
.sample-editor{width:min(800px,95vw)}.sample-editor label,.sample-tags{display:flex;flex-direction:column;gap:7px;margin:14px 0}.sample-editor .check-label{flex-direction:row}.sample-editor textarea{min-height:90px;background:var(--field-face);color:var(--field-ink);border:1px solid var(--line);padding:10px}.sample-waveform{position:relative;touch-action:none;cursor:crosshair}.sample-waveform canvas{display:block;width:100%;min-height:60px}.sample-waveform i{position:absolute;top:0;bottom:0;width:2px;background:var(--white);pointer-events:none}.sample-musical,.sample-playback{display:flex;gap:16px;align-items:center}.sample-musical label{flex:1;min-width:0}
</style>
