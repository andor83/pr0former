...
<div class="parameter-list"><input v-model="fullSearch" aria-label="Search all samples" placeholder="Search name, tags, category, key, BPM or author…"><p class="feature-note">Your samples and samples marked Global. Project-only samples from other authors stay available in the project sidebar.</p><p v-if="error" role="alert" class="field-error">{{error}}</p>
<article v-for="s in available" :key="s.id" class="sample-browser-row"><button class="button small" :aria-label="`Preview ${s.name}`" @click="play(s)">{{previewId===s.id?'Stop':'Preview'}}</button><button class="sample-name" :aria-label="`Edit sample ${s.name}`" @click="edit(s)">{{s.name}}<small>{{s.author}} · {{s.global?'Global':'Private'}} · {{s.channels}} ch · {{s.duration.toFixed(1)}} s<br>{{s.category}} {{s.tags}}</small></button><button class="button small" :disabled="!editable||busy||s.asset!==null" @click="add(s)">{{s.asset!==null?'In project':'Add to project'}}</button></article><p v-if="!available.length">No matching samples.</p>
</div></dialog>
