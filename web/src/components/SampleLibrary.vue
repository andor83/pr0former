...
<div class="parameter-list"><input v-model="fullSearch" aria-label="Search all samples" placeholder="Search name, tags, category, key, BPM or author…"><TagChips v-if="browserCloud.length" class="tag-filter" :tags="browserCloud.map(c=>c.tag)" :active="browserTags" :counts="counts(browserCloud)" interactive @toggle="tag=>browserTags=toggleTag(browserTags,tag)" /><p class="feature-note">Your samples and samples marked Global. Project-only samples from other authors stay available in the project sidebar. Click tags to filter; every selected tag must match.</p><p v-if="error" role="alert" class="field-error">{{error}}</p>
