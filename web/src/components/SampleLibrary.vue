...
<section class="sample-library" :class="{expanded:open}" aria-label="Sample library">
  ...
  <div v-if="open" id="sample-library-content" class="sample-library-content">
    ...
    <HelpNote>FFmpeg audio formats · up to 30 s / 8 channels / 64 MB. Converted at the current engine rate.</HelpNote>
  </div>
</section>
...
<div class="parameter-list"><input v-model="fullSearch" aria-label="Search all samples" placeholder="Search name, tags, category, key, BPM or author…"><TagChips v-if="browserCloud.length" class="tag-filter" :tags="browserCloud.map(c=>c.tag)" :active="browserTags" :counts="counts(browserCloud)" interactive @toggle="tag=>browserTags=toggleTag(browserTags,tag)" /><HelpNote>Your samples and samples marked Global. Project-only samples from other authors stay available in the project sidebar. Click tags to filter; every selected tag must match.</HelpNote><p v-if="error" role="alert" class="field-error">{{error}}</p>
