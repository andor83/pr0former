...
<template>
  <dialog ref="dialog" class="parameter-modal" aria-labelledby="save-subgraph-title" @cancel.prevent="!busy&&emit('close')">
    <header class="modal-header"><h2 id="save-subgraph-title">Save subgraph to library</h2><button class="icon-button" aria-label="Close subgraph save" :disabled="busy" @click="emit('close')">×</button></header>
    <div class="parameter-list"><label>Library name<input v-model="name" maxlength="256" :disabled="busy"></label><label>Save destination<select v-model="destination" :disabled="busy||loading" @change="destinationChanged"><option value="">New library entry (my own copy)</option><option v-for="item in entries" :key="item.id" :value="item.id">New version of {{item.name}}</option></select></label><label class="check-label"><input v-model="publish" type="checkbox" :disabled="busy||entry?.public">Make public for all users</label><HelpNote>Public entries cannot become private again. Each save creates an immutable version. Existing projects keep their inserted version. Earlier private versions stay private. To make a private variant, choose a new library entry.</HelpNote><HelpNote>Includes all nested nodes, connections, and uploaded WAV samples. Device routes are preserved.</HelpNote><p v-if="error" role="alert" class="field-error">{{error}}</p></div>
