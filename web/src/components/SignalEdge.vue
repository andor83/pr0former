...
const props = defineProps<EdgeProps<{ signal: string; channels: number; active: boolean; projectId:string; values?: Record<string, number> }>>()
...
const midiRows = computed(() => Object.entries(props.data?.values || {})
...
async function poll(token:number){
  ...
  if(!props.data?.active){channels.value=[];error.value='Enable the audio engine to visualize this signal.';return}
