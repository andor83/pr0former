...
<fieldset :disabled="!editable||!projectId||active||loading||!!busy"><label>Global sample rate (Hz)<select v-model.number="settings.sample_rate"><option :value="44100">44,100 Hz</option><option :value="48000">48,000 Hz</option><option :value="88200">88,200 Hz</option><option :value="96000">96,000 Hz</option></select></label><HelpNote>Clips are converted and cached at this rate. Originals are retained. Other projects prepare their clips when opened.</HelpNote>
<label>DSP block size<select v-model.number="settings.block_size"><option v-for="size in [32,64,128,256,512,1024]" :key="size" :value="size">{{size}} frames · {{(size/settings.sample_rate*1000).toFixed(2)}} ms</option></select></label><HelpNote>Smaller blocks increase scheduling overhead and reduce the target output queue. The device driver chooses its callback buffer separately.</HelpNote><h3>Active interfaces</h3><HelpNote>Checked outputs are available in Audio output node modals. With none selected, the engine serves browser monitors only.</HelpNote><p v-if="!settings.interfaces.length">No audio output interfaces detected.</p>
...
<HelpNote>Correction delays faster checked interfaces to match the largest entered latency. Separate device clocks may drift; an OS aggregate device is preferable for synchronized multichannel output.</HelpNote><h3>Enabled native inputs</h3><p v-if="!settings.input_interfaces.length">No native audio inputs are available on the server.</p><article v-for="i in settings.input_interfaces" :key="i.id" class="interface-card"><label class="check-label"><input v-model="i.enabled" type="checkbox" @change="toggleDevice('input',i)">{{i.name}}<span v-if="!devices?.input_interfaces?.some((d:any)=>d.id===i.id)"> · unavailable</span></label></article><HelpNote>Checked inputs are selectable in Audio input nodes. Saved inputs start automatically with the audio engine. All physical channels are available in Monitor; node routes can select channels 1–64.</HelpNote><button class="button primary" @click="save">Save system audio</button></fieldset>
