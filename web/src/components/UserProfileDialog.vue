...
<dialog ref="dialog" class="parameter-modal user-profile-dialog" aria-labelledby="user-profile-title" @cancel.prevent="!busy && emit('close')">
  ...
  <form class="profile-content" @submit.prevent="save">
    ...
    <fieldset v-if="profile" :disabled="busy">
      <section class="profile-avatar"><img v-if="avatar" :src="avatar" alt="Your avatar"><span v-else class="profile-initials">{{profile.username.slice(0,2).toUpperCase()}}</span><label>Upload avatar<input type="file" accept="image/png,image/jpeg,image/webp" @change="upload"><small>PNG, JPEG or WebP. Cropped to a 256px square and saved immediately.</small></label></section>
      ...
      <h3>Change password</h3><p v-if="profile.is_desktop_session">Your bundled session can set a password without the generated password. It will remain signed in.</p><p v-else>Enter your current password to choose a new one.</p>
