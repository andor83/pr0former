...
export function makeSubgraph(nodes: GraphNode[], edges: GraphEdge[], selection: string[], catalog: Descriptor[]) {
  ...
  const group: GraphNode = {id:crypto.randomUUID(),kind:'subgraph',label:'Subgraph',parent:chosen[0]!.parent||null,x:Math.min(...chosen.map(n=>n.x)),y:Math.min(...chosen.map(n=>n.y)),channels:2,parameters:{}}
  ...
  for(const n of chosen.filter(n=>n.kind.startsWith('subgraph_'))) {
    const input=n.kind.startsWith('subgraph_input_'), proxy={...JSON.parse(JSON.stringify(n)),id:crypto.randomUUID(),x:group.x+(input?-240:300)} as GraphNode
    nodes.push(proxy)
    ...
    added.push(input?{id:crypto.randomUUID(),source:proxy.id,source_port:'out',target:group.id,target_port:n.id}:{id:crypto.randomUUID(),source:group.id,source_port:n.id,target:proxy.id,target_port:'in'})
  }
  ...
  for (const edge of edges) {
    ...
    if (to) {
      ...
      added.push({id:crypto.randomUUID(),source:port,source_port:'out',target:edge.target,target_port:edge.target_port})
      edge.target=group.id;edge.target_port=port
    } else {
      const key=JSON.stringify([edge.source,edge.source_port]);let port=outputs.get(key)
      if(!port){port=boundary(edge.source,edge.source_port,'output');outputs.set(key,port);added.push({id:crypto.randomUUID(),source:edge.source,source_port:edge.source_port,target:port,target_port:'in'})}
