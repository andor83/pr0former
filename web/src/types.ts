...
export interface Staff {key_mode?:'major'|'minor'|null; clef_changes?:{beat:number;clef:string}[]; instrument_node?:string|null;midi_channel?:number|null;midi_port?:string|null; id:string; name:string; clef:string; key_signature?:string|null; transpose:number }
...
export interface Part { dynamics?:Dynamics|null; automation?:AutomationLane[]; staves?: Staff[]; id: string; name: string; performer: string | null; view: string; clef: string; key_signature?: string | null; show_time_signature?: boolean; notes: Note[]; loop_beats: number; instrument_node: string | null; midi_port?: string | null; midi_channel?: number; osc_destination?: string | null; osc_address: string }
export interface ScoreTimeline { version:1; length:number; loop_score:boolean; meters:{beat:number;beats:number;unit:number}[]; keys:{beat:number;key:string;mode?:'major'|'minor'|null}[]; repeats:{start:number;end:number;times:number;first_ending?:number|null}[]; navigation?:{at:number;target:number;fine?:number|null;coda?:[number,number]|null}|null }
