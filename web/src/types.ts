...
export interface NoteNotation {notehead?:string|null;drum_mark?:string|null;onset?:RationalTime|null;written_duration?:RationalTime|null; tie_to?:string|null;slur_to?:string|null;grace_to?:string|null;articulation?:string|null;octave?:number; staff:string; step:number; alter:number; voice:number; base:number; dots:number; tuplet_actual:number; tuplet_normal:number }
...
export interface Part { muted?:boolean; solo?:boolean; dynamics?:Dynamics|null; automation?:AutomationLane[]; staves?: Staff[]; performance_meters?:{beat:number;beats:number;unit:number}[]; id: string; name: string; performer: string | null; view: string; clef: string; key_signature?: string | null; show_time_signature?: boolean; notes: Note[]; loop_beats: number; instrument_node: string | null; midi_port?: string | null; midi_channel?: number; osc_destination?: string | null; osc_address: string }
...
export interface ConductedMidiBinding {action:string;target:string;source:"local"|"server";device:string;status:number;control:number}
export interface ConductedLayout {midi_bindings?:ConductedMidiBinding[];count_in_pulses:number;pulse_unit:number;sets:ConductedSet[]}
